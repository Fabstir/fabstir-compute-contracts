...
contract JobMarketplaceFABWithS5 is ReentrancyGuard {
    ...
    function withdrawTreasuryTokens(address token) external nonReentrant {
        require(msg.sender == treasuryAddress, "Only treasury");
        require(token != address(0), "Use withdrawTreasuryETH for ETH");
        ...
        require(IERC20(token).transfer(treasuryAddress, amount), "Token transfer failed");
    ...
    function emergencyWithdraw(address token) external nonReentrant {
        ...
        if (token == address(0)) {
            ...
            require(totalBalance > accumulatedTreasuryETH, "No stuck ETH");
            ...
            require(totalBalance > accumulated, "No stuck tokens");
