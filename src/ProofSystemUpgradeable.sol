 ...
 *
 * F202615147 (MEDIUM): Shared Proof Replay-Prevention State.
 * F202615149 (MEDIUM): Missing Access Control on verifyAndMarkComplete.
 * F202615002 (INFO): Unused Functions in ProofSystem.
 * F202615064 (INFO): Unfinalized Access-Control Functionality.
 * F202615065 (INFO): Unfinalized Circuits Functionality.
 * F202615066 (INFO): Redundant Function Wrapper.
 * Removed all dead functions (circuit registration, batch verification,
 * signature verification). Kept only markProofUsed() and setAuthorizedCaller().
 */
contract ProofSystemUpgradeable is Initializable, OwnableUpgradeable, UUPSUpgradeable, IProofSystem {
    ...
    mapping(bytes32 => bool) public verifiedProofs;       // Slot 0 (unchanged)

    // Storage placeholders for removed mappings — preserve slot alignment
    uint256 private __removed_registeredCircuits;          // Slot 1 (was: mapping(bytes32 => bool) registeredCircuits)
    uint256 private __removed_modelCircuits;                // Slot 2 (was: mapping(address => bytes32) modelCircuits)

    // Access control for markProofUsed - restricts to authorized callers only
    mapping(address => bool) public authorizedCallers;     // Slot 3 (preserved)

    // Events
    event ProofVerified(bytes32 indexed proofHash, address indexed prover, uint256 tokens);
    event AuthorizedCallerUpdated(address indexed caller, bool authorized);

    // Storage gap for future upgrades (unchanged from audit baseline)
    uint256[46] private __gap;                             // Slots 4-49
